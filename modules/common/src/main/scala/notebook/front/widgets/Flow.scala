...
case class LogPipe(
  ...
  size: (Int, Int) = (100, 60)) extends BoxPipeComponent[LogPipe]() {
  val name = "log"
...
trait Updatable {
  def update(
    varName: String,
    i: notebook.util.InterpreterUtil): Unit
}
...
case class CustomizableBoxPipe(
  ...
  position: (Int, Int) = (100, 100),
  size: (Int, Int) = (100, 60),
  parameters: Map[String, String] = Map("next" → "(a:Map[String, Any])=>a")) extends BoxPipeComponent[CustomizableBoxPipe]() with Updatable {
...
case class Flow() extends Updatable with JsWorld[PipeComponent[_], JsValue] {
  ...
  override def update(
    varName: String,
    i: notebook.util.InterpreterUtil): Unit = data.filter(_.isInstanceOf[Updatable])
  ...
  def run(init: (String /*BoxId*/ , List[String /*InPort*/ ]) => Map[String /*OutPort*/ , Any]): scala.collection.Map[(String, String), Any] = {
    ...
    val results = layers.map { pcs =>
      pcs.foreach { pc =>
        ...
        val valuesForLinksSource = linksToPc.map {
          case (inPort, (srcId, outPort)) =>
            for {
